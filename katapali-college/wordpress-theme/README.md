...
5. Click **Import Demo Content Now**. This creates, in one step:
   ...
   - 9 Quick Links shown on the homepage (RTI, SAMS, Sambalpur University, Anti Ragging Cell, Scholarship, etc.) plus 13 more across the footer's Resources and Useful Links columns (SWAYAM, e-Gyan Kosh, Shodhganga, CUET, NAAC, UGC, DHE, AISHE, National Scholarship Portal, Bargarh District website, etc.)
...
The design follows the look of a typical Indian government/degree-college site: decorative red-black-gold border strips top the header, above the footer, and below the organisation-logo strip; a thin contact/social bar sits above a centred header (logo on the left, college name and address centred with a coloured gradient heading, Admin Login on the right, logo size adjustable in the Customizer), followed by a dedicated white menu bar. The homepage below it follows a fixed order: a honeycomb hexagon photo hero (no slider - a static zig-zag grid of campus photos, so nothing shifts or auto-plays), a scrolling black "Latest Notice" ticker with a red badge, a compact About + Principal's Message pair of blue-title-bar cards, a three-column Notice / Tenders / Quick Links block (same blue-title-bar treatment), the faculty carousel, the student/faculty/department/years stats bar, and the photo gallery. An auto-scrolling strip of organisation logos (Ministry of Education, Election Commission, UGC, etc. — see **Organisation Logos** in the Katapali College menu) sits just above the footer on every page, and the footer itself is split into Reach Us At / Resources / Useful Links / Find Us on Map columns. Every one of those pieces is a normal editable post type, page, or Customizer field, and every section keeps a fixed column layout (equal-height cards) so nothing reflows or "spreads" between page loads.
